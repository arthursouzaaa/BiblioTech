...
export default function LivrosPage({ usuario }) {
  ...
  async function handleFazerEmprestimo(livroId) {
    try {
      const hoje = new Date().toISOString().split("T")[0];
      const devolucao = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split("T")[0];

      await api.post("/api/emprestimo", {
        livro_id: livroId,
      ...
      alert("📚 Empréstimo realizado com sucesso! Devolução prevista para daqui a 7 dias.");
    } catch (error) {
  ...
  async function handleSalvar(e) {
    ...
    // O backend espera: titulo, autor, categoria, ano
    const dados = { titulo, autor, categoria, ano: Number(ano) };

    try {
      if (itemEditando) {
        await api.put(`/api/livro/editar/${itemEditando.id}`, dados);
      } else {
        await api.post("/api/livro", dados);
      }
      await carregarLivros();
  ...
  async function handleConfirmarExclusao() {
    if (!isAdmin) return;
    try {
      await api.delete(`/api/livro/${itemExcluindoId}`);
      await carregarLivros();
  ...
  return (
    <div>
      <SectionHeader title="Gerenciar Livros" buttonLabel={isAdmin ? "Novo Livro" : null} onButtonClick={handleAbrirCriacao} />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gradient-to-r from-indigo-600 to-blue-700 text-white text-left">
              ...
              {isAdmin && <th className="py-3 px-4 font-semibold">Status</th>}
              <th className="py-3 px-4 font-semibold">Ações</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={isAdmin ? 6 : 5} className="py-6 text-center text-slate-400">Carregando...</td></tr>
            ) : (
              filtrados.map((l, i) => (
                <tr key={l.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                  ...
                  {isAdmin && <td className="py-3 px-4 text-slate-700"><StatusBadge status={l.status} /></td>}
                  <td className="py-3 px-4">
                    {isAdmin ? (
                      <ActionButtons onEdit={() => handleAbrirEdicao(l)} onDelete={() => handleAbrirExclusao(l.id)} />
                    ) : (
                      <button onClick={() => handleFazerEmprestimo(l.id)} className="px-3 py-1.5 rounded-lg bg-green-500 text-white text-xs font-semibold hover:bg-green-400 transition-colors">Emprestar</button>
                    )}
                  </td>
                </tr>
              ))
            )}
            {!loading && filtrados.length === 0 && (<tr><td colSpan={isAdmin ? 6 : 5} className="py-6 text-center text-slate-400">Nenhum livro encontrado.</td></tr>)}
          </tbody>
        </table>
      </div>

      {/* MODAL - Categoria e Ano estão aqui e prontos para uso */}
      {isAdmin && (
        <>
          <Modal title={itemEditando ? "Editar Livro" : "Novo Livro"} isOpen={modalAberto} onClose={() => setModalAberto(false)}>
          ...
          <Modal title="Confirmar Exclusão" isOpen={modalExcluirAberto} onClose={() => setModalExcluirAberto(false)}>
            <p className="text-slate-600 text-sm mb-6">Tem certeza que deseja excluir este registro? Esta ação não poderá ser desfeita.</p>
            <div className="flex gap-3">
              ...
              <button type="button" onClick={handleConfirmarExclusao} className="flex-1 bg-red-600 hover:bg-red-500 transition-colors text-white font-semibold py-2.5 rounded-xl">Excluir</button>
            </div>
          </Modal>
        </>
      )}
